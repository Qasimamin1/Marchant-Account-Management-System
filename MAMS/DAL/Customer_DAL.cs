            ...
            using (_connection._connection)
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    if (reader.HasRows)
                    {
                        ...
                        {
                            _customer = new Customer();
                            _customer.UID = Convert.ToInt32(reader[0]);
                            ...
                            _customer.Status = Convert.ToBoolean(reader[10].ToString());
                            //var dateExtract = Convert.ToDateTime(reader[2].ToString()).Date;
                            _customer.AddDate = Convert.ToDateTime(reader[11].ToString()).Date.ToString("dd-MM-yyyy");
                            _customer.AddedBy = reader[12].ToString();
